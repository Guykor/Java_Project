...
public class ClosedHashSetTest {
    private ClosedHashSet testSet;

    @Before
    public void createTestObjects() {
        testSet = new ClosedHashSet();
    }
    ...
    public void testDataConstructor() {
        ...
        testSet = new ClosedHashSet(stringsToAdd);
    }
//TEST BASIC OPERATION AND EDGE CASES - empty, full, maximal and minimal bounds, resizing recursively.
     ...
     * resizing part should take place before the element that surpass the upper bound has been added.
     * it's
    ...
    public void testResizingForMaximalBounds() {
        this.testSet = new ClosedHashSet(1, 0);
        ...
        for (String s : new String[]{"a", "b", "c", "d", "f", "e", "g", "h", "i", "j", "k", "l", "m", "n",
                "o", "p", "violationString!"}) {
            testSet.delete(s);
        }
        assertEquals(32, testSet.capacity());
    ...
    public void testResizingForMinimalBounds() {
        testSet = new ClosedHashSet(0, 0);
        fillSetTwelveStrings();
        assertEquals(12, testSet.size());
        assertEquals(65536, testSet.capacity()); //important! checks if the minimal size of the array is
        // pre
        // determined.
    }
